      ...
      <div className="flex flex-wrap gap-3 justify-center mb-8 bg-white/70 backdrop-blur-sm p-4 rounded-2xl shadow">
        {categories.map(cat => (
          <button
            ...
            className={`px-4 py-2 rounded-full font-medium transition ${active === cat ? "bg-orange-500 text-white shadow-lg" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
          >
            {cat}
          </button>
        ))}
      ...
      {selected && (
        ...
        >
          ...
          >
            ...
            <img src={selected.image} alt={selected.name} className="w-full h-60 object-cover rounded-xl mb-4" />
            <h2 className="text-2xl font-bold mb-2">{selected.name}</h2>
            <p className="text-orange-600 font-bold text-xl mb-3">Ksh {selected.price}</p>
            <p className="text-gray-600 mb-6">
              A delicious {selected.name.toLowerCase()} made with quality ingredients and crafted to perfection.
            </p>
            <button
              ...
              cart.map(item => (
                <div key={item.id} className="flex items-center justify-between mb-4 border-b pb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Ksh {item.price} × {item.qty}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setCart(prev => prev.map(p => p.id === item.id ? { ...p, qty: p.qty - 1 } : p).filter(p => p.qty > 0))}
            ...
            {cart.length > 0 && (
              <div className="mt-6 border-t pt-4">
                <h3 className="text-xl font-bold">
                  Total: Ksh {cart.reduce((sum, item) => sum + item.price * item.qty, 0)}
                </h3>
                <Link to="/checkout">
                  <button className="w-full bg-orange-600 text-white py-3 rounded-xl mt-4 hover:bg-orange-700 transition">
                    Checkout
                  </button>
                </Link>
              </div>
